...
def uep_cqs_78(gar, gar2):
    ...
    _dfs[f'CQS_GEP_{gar}'] = spark.table(f'CQS_GEP_{gar}').orderBy('Financiere_Adh', 'ID_Police', 'sit_prof_1', 'ID_Adh', 'Date_Dbt_Assce', 'Date_Fin_Assce', 'Date_Sin', 'Generation', 'Date_Rachat', 'Primes_{gar}', 'Rachat_{gar}', 'Date_term', 'term', 'term_expoff', 'Mois_fin_annee', 'Quotient_res', 'mois_rest', 'type_pret')
    _dfs[f'CQS_GEP_{gar}'].createOrReplaceTempView(f'CQS_GEP_{gar}')

    _dfs[f'CQS_UEP_PL__{gar}'] = spark.table(f'CQS_UEP_PL__{gar}').orderBy('Financiere_Adh', 'ID_Police', 'sit_prof_1', 'ID_Adh', 'Date_Dbt_Assce', 'Date_Fin_Assce', 'Date_Sin', 'Generation', 'Date_Rachat', 'Primes_{gar}', 'Rachat_{gar}', 'Date_term', 'term', 'term_expoff', 'Mois_fin_annee', 'Quotient_res', 'mois_rest', 'type_pret')
    _dfs[f'CQS_UEP_PL__{gar}'].createOrReplaceTempView(f'CQS_UEP_PL__{gar}')
    ...
    _dfs[f'CQS_GEP_{gar}_4'] = spark.table(f'CQS_GEP_{gar}_3')
        ...
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}0", F.col('Generation')))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}1", F.col('Generation') + 1))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}2", F.col('Generation') + 2))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}3", F.col('Generation') + 3))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}4", F.col('Generation') + 4))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}5", F.col('Generation') + 5))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}6", F.col('Generation') + 6))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}7", F.col('Generation') + 7))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}8", F.col('Generation') + 8))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}9", F.col('Generation') + 9))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}10", F.col('Generation') + 10))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}11", F.col('Generation') + 11))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}12", F.col('Generation') + 12))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}13", F.col('Generation') + 13))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}14", F.col('Generation') + 14))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"GEP_{gar2}15", F.col('Generation') + 15))
    ...
    _dfs[f'CQS_UEP_PL__{gar}_4'] = spark.table(f'CQS_UEP_PL__{gar}_3')
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}0", F.col('Generation')))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}1", F.col('Generation') + 1))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}2", F.col('Generation') + 2))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}3", F.col('Generation') + 3))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}4", F.col('Generation') + 4))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}5", F.col('Generation') + 5))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}6", F.col('Generation') + 6))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}7", F.col('Generation') + 7))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}8", F.col('Generation') + 8))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}9", F.col('Generation') + 9))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}10", F.col('Generation') + 10))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}11", F.col('Generation') + 11))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}12", F.col('Generation') + 12))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}13", F.col('Generation') + 13))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}14", F.col('Generation') + 14))
        # no ELSE: column is null when condition is false
        .withColumn('SURV', F.when(F.col('Variable') == f"UEP_PL_{gar2}15", F.col('Generation') + 15))
    ...
    _dfs[f'CQS_GEP_{gar}_5'] = (_dfs[f'CQS_GEP_{gar}_5']
        ...
        .withColumn('IDEAN', F.when(F.lit(gar2 == 'IARD'), F.expr("""cast(concat("1", substr(ID_Police,6,4)) as long)""")).otherwise(F.expr("""cast(concat("1", substr(ID_Police,1,4)) as long)""")))
    )
    # type_pret rajouté au HY24 pour la cession
