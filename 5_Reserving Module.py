...
_dfs[f'{pays}_CLMHDR_ALL'] = (_dfs[f'{pays}_CLMHDR_ALL']
    .withColumn('Cover', F.when(F.expr("""Country='SE' AND Cvr_Typ IN ('DK')"""), F.lit('DIS')).otherwise(F.col('Cover')))
)
_dfs[f'{pays}_CLMHDR_ALL'].createOrReplaceTempView(f'{pays}_CLMHDR_ALL')
...
_dfs[f'{pays}_FIRSTLASTBENEFIT'] = spark.sql(f"""SELECT Clm_Nmbr,
				...
				case when day(min(Trns_Dt))>Day(to_date({balancedate})) then case 
				...
				case when day(max(Trns_Dt))>Day(to_date({balancedate})) then case 
...
_dfs[f'Reserves_all_{cover_typ}_{pays}_V{yr_of_calculation}'] = (_dfs[f'Reserves_all_{cover_typ}_{pays}_V{yr_of_calculation}']
    ...
    .withColumn('Totl_Bnfts_Amnt_Pd', F.when(F.expr("""Totl_Bnfts_Amnt_Pd IS NULL"""), F.lit(0)).otherwise(F.col('Totl_Bnfts_Amnt_Pd')))
    .withColumn('Mnthly_Bnft', F.when(F.expr("""Mnthly_Bnft IS NULL"""), F.lit(0)).otherwise(F.col('Mnthly_Bnft')))
    .withColumn('Otstndng_Balnc', F.when(F.expr("""Otstndng_Balnc IS NULL"""), F.lit(0)).otherwise(F.col('Otstndng_Balnc')))
    .withColumn('Age_surv', F.expr("""floor(yrdif(Dt_of_Brth,Accdnt_Dt,ACTUAL))"""))
    .withColumn('Rsrv_Typ', F.when(F.expr("""Rsrv_Grp NOT IN ('ZZ1','ZZ2') AND STATUS IN ('CL','DC')"""), F.lit('CLOSE')).otherwise(F.col('Rsrv_Typ')))
    .withColumn('Rsrv_Typ',
    F.when(F.expr("""Rsrv_Grp IN ('ZZ1', 'ZZ2')     AND STATUS IN ('CL','DC')"""), F.lit('NON-CORE CLOSE'))
     .when(F.expr("""Rsrv_Grp NOT IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO') AND first_pd_date <=Date_of_reserving AND Totl_Bnfts_Amnt_Pd > 1"""), F.lit('ICOP'))
     .otherwise(F.col('Rsrv_Typ')))
    .withColumn('Rsrv_Typ',
    F.when(F.expr("""Rsrv_Grp NOT IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO') AND first_pd_date < Date_of_reserving AND Totl_Bnfts_Amnt_Pd < 1"""), F.lit('RBNP'))
     .when(F.expr("""Rsrv_Grp NOT IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO') AND first_pd_date > Date_of_reserving"""), F.lit('RBNP'))
     .when(F.expr("""Rsrv_Grp NOT IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO') AND Totl_Bnfts_Amnt_Pd = 0"""), F.lit('RBNP'))
     .when(F.expr("""Rsrv_Grp NOT IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO') AND cover IN ('CI','LIFE','PTD','GAP') AND Totl_Bnfts_Amnt_Pd > 0"""), F.lit('RBNP'))
     .when(F.expr("""Rsrv_Grp IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO')"""), F.lit('NON-CORE OPEN'))
     .otherwise(F.col('Rsrv_Typ')))
)
...
_dfs[f'Reserves_all_{cover_typ}_{pays}_V{yr_of_calculation}'] = (_dfs[f'Reserves_all_{cover_typ}_{pays}_V{yr_of_calculation}']
    .withColumn('Nmbr_Mnths_Pndng', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""((year(Date_of_reserving )-year(Rgstrtn_Dt))*12 + (month(Date_of_reserving)-month(Rgstrtn_Dt)))+1""")).otherwise(F.col('Nmbr_Mnths_Pndng')))
    .withColumn('Mnths_lag', F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.expr("""((year(Date_of_reserving )-year(trns_dt))*12 + (month(Date_of_reserving)-month(trns_dt)))""")).otherwise(F.col('Mnths_lag')))
    .withColumn('Mnths_lag', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""((year(Date_of_reserving )-year(Rgstrtn_Dt))*12 + (month(Date_of_reserving)-month(Rgstrtn_Dt)))+1""")).otherwise(F.col('Mnths_lag')))
    .withColumn('Nmbr_Mnths_Pndng',
    F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.lit(0))
     .when(F.expr("""Rsrv_Typ NOT IN ('ICOP', 'RBNP')"""), F.lit(0))
     .otherwise(F.col('Nmbr_Mnths_Pndng')))
    .withColumn('Mnths_lag', F.when(F.expr("""Rsrv_Typ NOT IN ('ICOP','RBNP')"""), F.lit(0)).otherwise(F.col('Mnths_lag')))
    .withColumn('Nmbr_Mnths_Pndng2', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""((year(Date_of_reserving )-year(Rgstrtn_Dt))*12 + (month(Date_of_reserving)-month(Rgstrtn_Dt)))+1""")).otherwise(F.col('Nmbr_Mnths_Pndng2')))
    .withColumn('Mnths_lag2', F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.expr("""((year(Date_of_reserving )-year(trns_dt))*12 + (month(Date_of_reserving)-month(trns_dt)))""")).otherwise(F.col('Mnths_lag2')))
    .withColumn('Mnths_lag2', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""((year(Date_of_reserving )-year(Rgstrtn_Dt))*12 + (month(Date_of_reserving)-month(Rgstrtn_Dt)))+1""")).otherwise(F.col('Mnths_lag2')))
    .withColumn('Nmbr_Mnths_Pndng2',
    F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.lit(0))
     .when(F.expr("""Rsrv_Typ NOT IN ('ICOP', 'RBNP')"""), F.lit(0))
     .otherwise(F.col('Nmbr_Mnths_Pndng2')))
    .withColumn('Mnths_lag2', F.when(F.expr("""Rsrv_Typ  NOT IN ('ICOP','RBNP')"""), F.lit(0)).otherwise(F.col('Mnths_lag2')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.expr("""floor(Totl_Bnfts_Amnt_Pd/Mnthly_Bnft+0.5)""")).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('Nmbr_Bnfts_Pd2', F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.expr("""floor(Totl_Bnfts_Amnt_Pd/Mnthly_Bnft+0.5)""")).otherwise(F.col('Nmbr_Bnfts_Pd2')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('Nmbr_Bnfts_Pd2', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd2')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Rsrv_Typ = 'CLOSE'"""), F.expr("""floor(Totl_Bnfts_Amnt_Pd/Mnthly_Bnft+0.5)""")).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('Nmbr_Bnfts_Pd2', F.when(F.expr("""Rsrv_Typ = 'CLOSE'"""), F.expr("""floor(Totl_Bnfts_Amnt_Pd/Mnthly_Bnft+0.5)""")).otherwise(F.col('Nmbr_Bnfts_Pd2')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Nmbr_Bnfts_Pd IS NULL"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('Nmbr_Bnfts_Pd2', F.when(F.expr("""Nmbr_Bnfts_Pd2 IS NULL"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd2')))
)
...
_dfs[f'Reserves_{cover_typ}_{pays}_V{yr_of_calculation}_F'] = (_dfs[f'Reserves_{cover_typ}_{pays}_V{yr_of_calculation}_F']
    .withColumn('Gndr',
    F.when(F.expr("""Gndr =''"""), F.lit('M'))
     .when(F.expr("""Gndr ='X'"""), F.lit('M'))
     .otherwise(F.col('Gndr')))
    .withColumn('Age_surv',
    F.when(F.expr("""(Age_surv IS NULL OR Age_surv IN (0))"""), F.col('AVG_AGE'))
     .when(F.expr("""Age_surv <= 0"""), F.col('AVG_AGE'))
     .otherwise(F.col('Age_surv')))
    .withColumn('potential_clm_amt', F.when(F.expr("""potential_clm_amt=0"""), F.lit(0)).otherwise(F.col('potential_clm_amt')))
    .withColumn('Otstndng_Balnc', F.when(F.expr("""Otstndng_Balnc=0"""), F.lit(0)).otherwise(F.col('Otstndng_Balnc')))
    .withColumn('Mnthly_Bnft', F.when(F.expr("""Mnthly_Bnft=0"""), F.lit(0)).otherwise(F.col('Mnthly_Bnft')))
    .withColumn('Nmbr_Mnths_Pndng', F.when(F.expr("""Nmbr_Mnths_Pndng > 12"""), F.lit(12)).otherwise(F.col('Nmbr_Mnths_Pndng')))
    .withColumn('Mnths_lag', F.when(F.expr("""Mnths_lag > 12"""), F.lit(12)).otherwise(F.col('Mnths_lag')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Nmbr_Bnfts_Pd > 60"""), F.lit(60)).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('Age_surv',
    F.when(F.expr("""cover='IU' AND Age_surv > 80"""), F.lit(70))
     .when(F.expr("""cover IN ('DIS', 'LIFE','CI','PTD','GAP') AND Age_surv > 100"""), F.lit(100))
     .otherwise(F.col('Age_surv')))
)
...
if cover_typ== 'IU'  or  cover_typ== 'DIS':
    import_excel(file=import_01, out=f"Table_duration_{cover_typ}_{pays}", onglet="Duration")
    import_excel(file=import_01, out=f"Table_acceptation_{cover_typ}_{pays}", onglet="Acceptation")
    # MISE EN FORME DES FACTEURS DE LA TABLE DURATION ET DE LA TABLE ACCEPTATION
    _dfs[f'TABLE_DURATION_{cover_typ}_{pays}'] = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    _dfs[f'TABLE_DURATION_{cover_typ}_{pays}'].createOrReplaceTempView(f'TABLE_DURATION_{cover_typ}_{pays}')

    facteur_age_D = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    facteur_age_D = facteur_age_D.select('Age_surv', 'factor_age_D')
    facteur_age_D.createOrReplaceTempView('facteur_age_D')

    facteur_gender_D = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    facteur_gender_D = facteur_gender_D.select('Gndr', 'factor_sexe_D', 'Intercept_D')
    facteur_gender_D.createOrReplaceTempView('facteur_gender_D')

    facteur_Number_D = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    facteur_Number_D = facteur_Number_D.select('Nmbr_Bnfts_Pd', 'factor_numbr_D')
    facteur_Number_D.createOrReplaceTempView('facteur_Number_D')

    facteur_Inactivity_D = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    facteur_Inactivity_D = facteur_Inactivity_D.select('Mnths_lag', 'factor_lag_D')
    facteur_Inactivity_D.createOrReplaceTempView('facteur_Inactivity_D')

    facteur_MAX_D = spark.table(f'TABLE_DURATION_{cover_typ}_{pays}')
    facteur_MAX_D = facteur_MAX_D.select('Max_Nmbr_Bnfts', 'factor_max_D')
    facteur_MAX_D.createOrReplaceTempView('facteur_MAX_D')

    _dfs[f'TABLE_ACCEPTATION_{cover_typ}_{pays}'] = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{pays}')
    _dfs[f'TABLE_ACCEPTATION_{cover_typ}_{pays}'].createOrReplaceTempView(f'TABLE_ACCEPTATION_{cover_typ}_{pays}')

    facteur_age_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{pays}')
    facteur_age_A = facteur_age_A.select('Age_surv', 'factor_age_A')
    facteur_age_A.createOrReplaceTempView('facteur_age_A')

    facteur_gender_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{pays}')
    facteur_gender_A = facteur_gender_A.select('Gndr', 'factor_sexe_A', 'Intercept_A')
    facteur_gender_A.createOrReplaceTempView('facteur_gender_A')

    facteur_Waiting_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{pays}')
    facteur_Waiting_A = facteur_Waiting_A.select('Nmbr_Mnths_Pndng', 'factor_month_A')
    facteur_Waiting_A.createOrReplaceTempView('facteur_Waiting_A')
...
if cover_typ== 'IU'  or  cover_typ== 'DIS':
    _dfs[f'Reserves_{cover_typ}_{pays}_V{yr_of_calculation}_F'] = spark.sql(f"""Select distinct t1.*,t2.factor_age_D,t3.factor_numbr_D,t4.factor_max_D,t5.factor_sexe_D,t5.Intercept_D,t6.factor_lag_D,t7.factor_age_A,t8.factor_sexe_A,t8.Intercept_A,t9.factor_month_A
...
if cover_typ== 'LIFE'  or  cover_typ== 'CI'  or  cover_typ== 'PTD'  or  cover_typ== 'GAP':
    _dfs[f'Reserves_{cover_typ}_{pays}_V{yr_of_calculation}_F'] = spark.sql(f"""Select distinct t1.*,0 as factor_age_D,0 as factor_numbr_D,0 as factor_max_D,0 as factor_sexe_D,0 as Intercept_D,0 as factor_lag_D,t7.factor_age_A,t8.factor_sexe_A,t8.Intercept_A,t9.factor_month_A
...
_dfs[f'CLMHDR_{cover_typ}_{pays}_V{yr_of_calculation}'] = (_dfs[f'CLMHDR_{cover_typ}_{pays}_V{yr_of_calculation}']
    .withColumn('Nmbr_Bnfts_Otstndng',
    F.when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.expr("""exp(Intercept_D + factor_age_D + factor_sexe_D + factor_max_D+ factor_numbr_D + factor_lag_D)"""))
     .when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""exp(Intercept_D + factor_age_D + factor_sexe_D + factor_max_D+ factor_numbr_D + factor_lag_D)"""))
     .otherwise(F.col('Nmbr_Bnfts_Otstndng')))
    .withColumn('Probablty_Otstndng', F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""exp(Intercept_A + factor_age_A + factor_sexe_A + factor_month_A)/(1+exp(Intercept_A+factor_age_A +factor_sexe_A+ factor_month_A))""")).otherwise(F.col('Probablty_Otstndng')))
    .withColumn('Nmbr_Bnfts_Otstndng', F.when(F.expr("""Rsrv_Typ IN ('CLOSE','NON-CORE OPEN','NON-CORE CLOSE')"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Otstndng')))
    .withColumn('Probablty_Otstndng', F.when(F.expr("""Rsrv_Typ IN ('CLOSE','ICOP','NON-CORE OPEN','NON-CORE CLOSE')"""), F.lit(0)).otherwise(F.col('Probablty_Otstndng')))
    .withColumn('Rsrv_Amt', F.when(F.expr("""Rsrv_Typ = 'ICOP' AND cover IN ('IU','DIS')"""), F.expr("""Mnthly_Bnft*Nmbr_Bnfts_Otstndng""")).otherwise(F.col('Rsrv_Amt')))
    .withColumn('Rsrv_Amt',
    F.when(F.expr("""Rsrv_Typ = 'RBNP' AND cover IN ('IU', 'DIS')"""), F.expr("""Probablty_Otstndng*Mnthly_Bnft*Nmbr_Bnfts_Otstndng"""))
     .when(F.expr("""Rsrv_Typ NOT IN ('ICOP', 'RBNP')"""), F.lit(0))
     .otherwise(F.col('Rsrv_Amt')))
    .withColumn('Nmbr_Bnfts_Otstndng', F.lit(0))
    .withColumn('Probablty_Otstndng',
    F.when(F.expr("""Rsrv_Typ = 'RBNP'"""), F.expr("""exp(Intercept_A + factor_age_A + factor_sexe_A + factor_month_A)/(1+exp(Intercept_A+factor_age_A +factor_sexe_A+ factor_month_A))"""))
     .when(F.expr("""Rsrv_Typ = 'ICOP'"""), F.lit(0))
     .when(F.expr("""Rsrv_Typ  NOT IN ('RBNP')"""), F.lit(0))
     .otherwise(F.col('Probablty_Otstndng')))
    .withColumn('Rsrv_Amt', F.when(F.expr("""Rsrv_Typ = 'RBNP' AND cover IN ('CI','LIFE','PTD')"""), F.expr("""Probablty_Otstndng*Otstndng_Balnc""")).otherwise(F.col('Rsrv_Amt')))
    .withColumn('Rsrv_Amt',
    F.when(F.expr("""Rsrv_Typ = 'RBNP' AND cover IN ('GAP')"""), F.expr("""Probablty_Otstndng*potential_clm_amt"""))
     .when(F.expr("""Rsrv_Typ  NOT IN ('ICOP', 'RBNP')"""), F.lit(0))
     .when(F.expr("""Rsrv_Grp IN ('ZZ1', 'ZZ2')"""), F.lit(0))
     .otherwise(F.col('Rsrv_Amt')))
)
...
def fusion(pays, yr_of_calculation):
    ...
    _dfs[f'CLMHDR_Hors_p_{pays}_V{yr_of_calculation}'] = (_dfs[f'CLMHDR_Hors_p_{pays}_V{yr_of_calculation}']
        .withColumn('Rsrv_Typ', F.lit(None).cast(StringType()))  # LENGTH Rsrv_Typ $40
        .withColumn('Rsrv_Typ',
        F.when(F.expr("""Rsrv_Grp IN ('ZZ1', 'ZZ2') AND STATUS IN ('OP','RO')"""), F.lit('NON-CORE OPEN'))
         .when(F.expr("""Rsrv_Grp IN ('ZZ1', 'ZZ2') AND STATUS IN ('CL','DC')"""), F.lit('NON-CORE CLOSE'))
         .otherwise(F.col('Rsrv_Typ')))
        .withColumn('Date_of_reserving', F.expr(f"""make_date({month_reserving}, {day_reserving}, {yr_reserving})"""))
        .withColumn('Nmbr_Mnths_Pndng', F.when(F.expr("""Rsrv_Grp  IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Nmbr_Mnths_Pndng')))
        .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Rsrv_Grp  IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd')))
        .withColumn('Nmbr_Bnfts_Otstndng', F.when(F.expr("""Rsrv_Grp  IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Otstndng')))
        .withColumn('Probablty_Otstndng', F.when(F.expr("""Rsrv_Grp  IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Probablty_Otstndng')))
        .withColumn('Rsrv_Amt', F.when(F.expr("""Rsrv_Grp  IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Rsrv_Amt')))
    )
    # FORMAT/INFORMAT: FORMAT Date_of_reserving DDMMYY10.
    ...
    _dfs[f'CLMHDR_all_{pays}'] = spark.sql(f"""SELECT 
    		...
    		GROUP BY Clm_Nmbr """)
    _dfs[f'CLMHDR_all_{pays}'].createOrReplaceTempView(f'CLMHDR_all_{pays}')
    _dfs[f'CLMHDR_all_{pays}'].write.mode('overwrite').saveAsTable(f'{ouput}.CLMHDR_all_{pays}')
...
_dfs[f'CLMHDR_all_{pays}_CR'] = (_dfs[f'CLMHDR_all_{pays}_CR']
    ...
    F.when(F.expr("""QP_rei_CLAIM IS NULL"""), F.lit(0))
     .when(F.expr("""QP_rei_CLAIM NOT IS NULL"""), F.lit(4))
     .otherwise(F.col('Type_Insurance')))
    .withColumn('Rsrv_Amt', F.when(F.expr("""Rsrv_Grp IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Rsrv_Amt')))
    .withColumn('Nmbr_Bnfts_Otstndng', F.when(F.expr("""Rsrv_Grp IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Otstndng')))
    .withColumn('Probablty_Accptd', F.when(F.expr("""Rsrv_Grp IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Probablty_Accptd')))
    .withColumn('Nmbr_Bnfts_Pd', F.when(F.expr("""Rsrv_Grp IN ('ZZ1','ZZ2')"""), F.lit(0)).otherwise(F.col('Nmbr_Bnfts_Pd')))
    .withColumn('QP_rei_CLAIM', F.when(F.expr("""Type_Insurance=0"""), F.lit(1)).otherwise(F.col('QP_rei_CLAIM')))
)
...
_dfs[f'CLMHDR_all_{pays}_CR'] = spark.sql(f"""SELECT 		
		...
		GROUP BY Clm_Nmbr """)
_dfs[f'CLMHDR_all_{pays}_CR'].createOrReplaceTempView(f'CLMHDR_all_{pays}_CR')
_dfs[f'CLMHDR_all_{pays}_CR'].write.mode('overwrite').saveAsTable(f'{ouput}.CLMHDR_all_{pays}_CR')
...
CLMHDR_all_GR_CR = (CLMHDR_all_GR_CR
    .withColumn('Rsrv_Amt_Gross', F.when(F.expr("""Country = 'GR' AND SCHEME IN ('BP3.3', 'BP3.4', 'BP5.3', 'BP5.4', 'BP7.3', 'BP7.4')"""), F.lit(0)).otherwise(F.col('Rsrv_Amt_Gross')))
    .withColumn('Rsrv_Amt_Net', F.when(F.expr("""Country = 'GR' AND SCHEME IN ('BP3.3', 'BP3.4', 'BP5.3', 'BP5.4', 'BP7.3', 'BP7.4')"""), F.lit(0)).otherwise(F.col('Rsrv_Amt_Net')))
)
...
CLMHDR_all_GR = (CLMHDR_all_GR
    .withColumn('Rsrv_Amt', F.when(F.expr("""Country = 'GR' AND Schm IN ('BP3.3', 'BP3.4', 'BP5.3', 'BP5.4', 'BP7.3', 'BP7.4')"""), F.lit(0)).otherwise(F.col('Rsrv_Amt')))
)
...
if cover_typ== 'IU'  or  cover_typ== 'DIS':
    import_excel(file=import_01, out=f"Table_duration_{cover_typ}_{type_prod}_{pays}", onglet="Duration")
    import_excel(file=import_01, out=f"Table_acceptation_{cover_typ}_{type_prod}_{pays}", onglet="Acceptation")
    # MISE EN FORME DES FACTEURS DE LA TABLE DURATION ET DE LA TABLE ACCEPTATION
    _dfs[f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}'] = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    _dfs[f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}'].createOrReplaceTempView(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')

    facteur_age_D = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    facteur_age_D = facteur_age_D.select('Age_surv', 'factor_age_D')
    facteur_age_D.createOrReplaceTempView('facteur_age_D')

    facteur_gender_D = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    facteur_gender_D = facteur_gender_D.select('Gndr', 'factor_sexe_D', 'Intercept_D')
    facteur_gender_D.createOrReplaceTempView('facteur_gender_D')

    facteur_Number_D = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    facteur_Number_D = facteur_Number_D.select('Nmbr_Bnfts_Pd', 'factor_numbr_D')
    facteur_Number_D.createOrReplaceTempView('facteur_Number_D')

    facteur_Inactivity_D = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    facteur_Inactivity_D = facteur_Inactivity_D.select('Mnths_lag', 'factor_lag_D')
    facteur_Inactivity_D.createOrReplaceTempView('facteur_Inactivity_D')

    facteur_MAX_D = spark.table(f'TABLE_DURATION_{cover_typ}_{type_prod}_{pays}')
    facteur_MAX_D = facteur_MAX_D.select('Max_Nmbr_Bnfts', 'factor_max_D')
    facteur_MAX_D.createOrReplaceTempView('facteur_MAX_D')

    _dfs[f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}'] = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}')
    _dfs[f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}'].createOrReplaceTempView(f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}')

    facteur_age_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}')
    facteur_age_A = facteur_age_A.select('Age_surv', 'factor_age_A')
    facteur_age_A.createOrReplaceTempView('facteur_age_A')

    facteur_gender_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}')
    facteur_gender_A = facteur_gender_A.select('Gndr', 'factor_sexe_A', 'Intercept_A')
    facteur_gender_A.createOrReplaceTempView('facteur_gender_A')

    facteur_Waiting_A = spark.table(f'TABLE_ACCEPTATION_{cover_typ}_{type_prod}_{pays}')
    facteur_Waiting_A = facteur_Waiting_A.select('Nmbr_Mnths_Pndng', 'factor_month_A')
    facteur_Waiting_A.createOrReplaceTempView('facteur_Waiting_A')
...
# MERGE DES FACTEURS DE LA TABLE DURATION ET DE TABLE ACCEPTATION
if cover_typ== 'IU'  or  cover_typ== 'DIS':
    _dfs[f'Reserves_{cover_typ}_{type_prod}_{pays}_V{yr_of_calculation}_F'] = spark.sql(f"""Select distinct t1.*,t2.factor_age_D,t3.factor_numbr_D,t4.factor_max_D,t5.factor_sexe_D,t5.Intercept_D,t6.factor_lag_D,t7.factor_age_A,t8.factor_sexe_A,t8.Intercept_A,t9.factor_month_A
...
if cover_typ== 'LIFE'  or  cover_typ== 'CI'  or  cover_typ== 'PTD'  or  cover_typ== 'GAP':
    _dfs[f'Reserves_{cover_typ}_{type_prod}_{pays}_V{yr_of_calculation}_F'] = spark.sql(f"""Select distinct t1.*,0 as factor_age_D,0 as factor_numbr_D,0 as factor_max_D,0 as factor_sexe_D,0 as Intercept_D,0 as factor_lag_D,t7.factor_age_A,t8.factor_sexe_A,t8.Intercept_A,t9.factor_month_A
